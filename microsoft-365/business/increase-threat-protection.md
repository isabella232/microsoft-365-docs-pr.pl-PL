...
Ten artykuł pomaga zwiększyć ochronę w subskrypcji Microsoft 365 w celu ochrony przed phishingiem, złośliwym oprogramowaniem i innymi zagrożeniami. Zalecenia te są odpowiednie dla organizacji o zwiększonej potrzebie bezpieczeństwa, jak kancelarie prawne i kliniki opieki zdrowotnej.

Zanim zaczniesz, Sprawdź swój pakiet Office 365 bezpieczny wynik. Pakiet Office 365 bezpieczny wynik analizuje zabezpieczenia organizacji pakietu Office 365 w oparciu o zwykłe działania i ustawienia zabezpieczeń oraz przypisuje wynik. Zacznij od Odnotuj swój aktualny wynik. Aby zwiększyć swój wynik, wykonaj czynności zalecane w tym artykule. Celem nie jest osiągnięcie maksymalnego wyniku, ale zdawać sobie sprawę z możliwości ochrony środowiska, które nie wpływają negatywnie na produktywność użytkowników. 
...
Środowisko Office 365 lub Microsoft 365 obejmuje ochronę przed złośliwym oprogramowaniem. Ochronę można zwiększyć, blokując załączniki z typami plików, które są powszechnie używane w przypadku złośliwego oprogramowania. Aby zwiększyć ochronę przed złośliwym oprogramowaniem w wiadomościach e-mail:
...
5. W obszarze **typowe typy załączników filtru**, wybierz opcję **wł**. Typy plików, które są blokowane są wyświetlane w oknie bezpośrednio pod tym formantem. Upewnij się, że dodajesz następujące typy plików:
   - ADE, ADP, ani, bas, nietoperz, chm, cmd, com, cpl, CRT, HLP, HT, HTA, inf, ins, ISP, praca, js, JSE, lnk, MDA, MDB, MDE, MDZ, msc, MSI, msp, MST, PCD, reg, SCR, SCT, SHS, URL, VB, VBE, vbs, WSC, wsf, WSH, exe, PIF  <br/> 
...
Oprogramowanie ransomware ogranicza dostęp do danych przez szyfrowanie plików lub blokowanie ekranów komputera. Następnie próbuje wyłudzić pieniądze od ofiar, prosząc o "okup", zwykle w postaci kryptowalut, takich jak Bitcoin, w zamian za dostęp do danych. 
  
Aby zabezpieczyć się przed oprogramowaniem typu ransomware, Utwórz co najmniej jedną reguły przepływu poczty, aby zablokować rozszerzenia plików, które są powszechnie używane w przypadku oprogramowania ransomware. (Dodano te zasady w [podnieść poziom ochrony przed złośliwym oprogramowaniem w kroku poczty](#raise-the-level-of-protection-against-malware-in-mail) .) Możesz także ostrzec użytkowników, którzy otrzymają te załączniki w wiadomości e-mail.

Oprócz plików zablokowanych w poprzednim kroku dobrym rozwiązaniem jest utworzenie reguły ostrzegające użytkowników przed otwarciem załączników plików pakietu Office, które zawierają makra. Ransomware mogą być ukryte wewnątrz makra, więc ostrzegają użytkowników, aby nie otwierać tych plików od osób, których nie znają.

Aby utworzyć regułę transportu poczty:
  
1. <a href="https://go.microsoft.com/fwlink/p/?linkid=837890" target="_blank">https://admin.microsoft.com</a>Przejdź do centrum administracyjnego i wybierz **wymiany** **centrów** \> administracyjnych.
    
2. W kategorii **przepływ poczty** wybierz **reguły**.
    
3. Wybierz **+**, a następnie wybierz **Utwórz nową regułę**.
    
4. Wybierz **więcej opcji** w dolnej części okna dialogowego, aby wyświetlić pełny zestaw opcji. 
...
|Podaj tekst komunikatu  <br/> |Nie otwieraj tych typów plików od osób, których nie znasz, ponieważ mogą one zawierać makra ze złośliwym kodem.  <br/> |
...
Hakerzy, którzy uzyskują dostęp do skrzynki pocztowej użytkownika mogą ukraść pocztę przez ustawienie skrzynki pocztowej, aby automatycznie przesyłać dalej wiadomości e-mail. Może się to zdarzyć nawet bez świadomości użytkownika. Aby temu zapobiec, skonfiguruj regułę przepływu poczty. 
...
1. W centrum administracyjnym Microsoft 365 wybierz pozycję Exchange **Centers** \> ****.
    
2. W kategorii **przepływ poczty** wybierz **reguły**.
    
3. Wybierz **+**, a następnie wybierz **Utwórz nową regułę**.
    
4. Aby wyświetlić wszystkie opcje, wybierz **więcej opcji** w dolnej części okna dialogowego. 
...
5. Określ nazwę, opis i ustawienia dla zasad zgodnie z zaleceniami w poniższej tabeli. Aby uzyskać więcej informacji, zobacz informacje [o opcjach zasad ochrony przed phishingiem ATP](https://docs.microsoft.com/microsoft-365/security/office-365-security/set-up-anti-phishing-policies#learn-about-atp-anti-phishing-policy-options). 
...
1. Na stronie bezpieczne łącza w obszarze **zasady, które mają zastosowanie do całej organizacji**, wybierz **+** opcję, aby utworzyć nowe zasady. 
